...
void addNiceNodes(int level, int currentLevel, int left, int right, CP::map_bst<int, int> &bst)
{
    ...
    bst[(left + right) / 2] = 69420;

    addNiceNodes(level, currentLevel + 1, left, (left + right) / 2, bst);
    addNiceNodes(level, currentLevel + 1, (left + right) / 2, right, bst);
}
...
void gen_best_bst(int n, CP::map_bst<int, int> &bst)
{
    ...
    addNiceNodes(level, 0, 1, n + 1, bst);
